...
public final class StructurePiece {
    ...
    /**
     * Returns the local Y coordinate of the lowest layer that contains at least
     * one non-air block.
     *
     * <p>The origin passed to {@link IStructureLoader#place} is the world-space
     * position of the structure's {@code (minX, minY, minZ)} corner, which
     * corresponds to local Y=0.  If the selection box extended below the actual
     * ground surface, local Y=0 is air and the first real blocks appear at a
     * higher local Y.  Placing without accounting for this offset causes the
     * structure to float above the target position.
     *
     * <p>To place a structure flush with a target ground level {@code groundY}:
     * <pre>
     *   BlockPos origin = new BlockPos(x, groundY - piece.groundOffset(), z);
     *   loader.place(world, piece, origin);
     * </pre>
     *
     * @return Local Y of the first non-air layer, or 0 if the structure is
     *         entirely air or its bottom layer already contains blocks.
     */
        ...
        for (int ry = 0; ry < sY; ry++) {
            for (int rz = 0; rz < sZ; rz++) {
                for (int rx = 0; rx < sX; rx++) {
                    if (!isAir(blockEntryAt(rx, ry, rz).blockStateId())) {
                        return ry;
                    }
                }
            }
    ...
    /** Mirrors the air check in the internal placement engine. */
    private static boolean isAir(String blockStateId) {
        int b = blockStateId.indexOf('[');
        String base = b >= 0 ? blockStateId.substring(0, b) : blockStateId;
        return base.equals("minecraft:air")
            || base.equals("minecraft:cave_air")
            || base.equals("minecraft:void_air");
    }
